...
Condensed from `.claude/projects/felsqualle.com-wordpress-theme/memory/*.md`
(a prior Claude Code session's notes) on 2026-08-13. Facts here supplement
CLAUDE.md rather than repeat it — check CLAUDE.md first.
...
- The footer menu (`wp_navigation` post 118, "Footer") is empty (verified
  ...
  pages exist, or core's fallback renders the full page list — see CLAUDE.md's
  note on empty navigation menus.
...
- `.claude/settings.json` is untracked/gitignored, holds one-off Bash
  allowlist entries.
...
- The "Known issues 3 and 4" the old notes describe as unfixed were cleared
  from CLAUDE.md's Known issues section entirely on 2026-08-13, per explicit
  user instruction — current code is treated as authoritative until issues
  are manually reassigned. Don't reintroduce that tracking speculatively.
- The `no-title` custom template and its `patterns/site-search.php` companion
  ...
  2026-08-13, and the `patterns/` directory was dropped earlier (CLAUDE.md:
  "the theme's four patterns were dropped in favour of the ones core ships").
